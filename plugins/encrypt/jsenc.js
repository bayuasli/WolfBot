    ...
    try {
      ...
      function sbyuxdProtect(code) {
        ...
        res = res.replace(/function\s+([a-zA-Z_$][a-zA-Z0-9_$]*)/g, (m, n) => {
          if (!map[n]) map[n] = "_0x" + (++fcnt).toString(16) + Math.random().toString(36).slice(2, 6);
          return "function " + map[n];
        });

        res = res.replace(/\b(var|let|const)\s+([a-zA-Z_$][a-zA-Z0-9_$]*)/g, (m, kw, n) => {
          if (!map[n]) map[n] = "_0x" + (++vcnt).toString(16) + Math.random().toString(36).slice(2, 6);
          return kw + " " + map[n];
        });
