  ...
  run: async (conn, m, { quoted }) => {
    ...
    if (!method) {
      ...
      return new Button(conn)
        ...
        .makeRow('', 'Dynamic Key (Rekomendasi)', 'Key & IV random per file, AES-256-GCM 2 layer', '.enchtml pro')
        .makeRow('', 'Fixed Key', 'Key tetap sama tiap file, AES-256-GCM 2 layer', '.enchtml fix')
        .send(m.chat, { quoted: m })
    }

    const cached = pendingHtml.get(m.sender)
    if (!cached) {
      return m.reply('Sesi enkripsi kadaluarsa atau nggak ditemukan. Reply file .html lagi dari awal.')
    }
