...
async function createRepo(name, isPrivate, description) {
  const { data } = await axios.post(`${GH_API}/user/repos`, {
    name,
    private: isPrivate,
    description,
    auto_init: true
...
async function deleteRepo(user, repo) {
  await axios.delete(`${GH_API}/repos/${user}/${repo}`, { headers: getHeaders() })
}

async function renameRepo(user, oldName, newName) {
  const { data } = await axios.patch(`${GH_API}/repos/${user}/${oldName}`, {
    name: newName
  }, { headers: getHeaders() })
  return data
}
...
async function getRepo(user, repo) {
  const { data } = await axios.get(`${GH_API}/repos/${user}/${repo}`, { headers: getHeaders() })
  return data
}

async function setVisibility(user, repo, isPrivate) {
  const { data } = await axios.patch(`${GH_API}/repos/${user}/${repo}`, {
    private: isPrivate
  }, { headers: getHeaders() })
  return data
}
...
async function ensureRepo(owner, repo) {
  try {
    await axios.get(`${GH_API}/repos/${owner}/${repo}`, { headers: getHeaders() })
  } catch {
    await axios.post(`${GH_API}/user/repos`, {
      name: repo,
      private: false,
      auto_init: true
    }, { headers: getHeaders() })
...
async function getRateLimit() {
  const { data } = await axios.get(`${GH_API}/rate_limit`, { headers: getHeaders() })
  return data
}
...
async function starRepo(owner, repo) {
  await axios.put(`${GH_API}/user/starred/${owner}/${repo}`, {}, { headers: getHeaders() })
}

async function unstarRepo(owner, repo) {
  await axios.delete(`${GH_API}/user/starred/${owner}/${repo}`, { headers: getHeaders() })
}

async function forkRepo(owner, repo) {
  const { data } = await axios.post(`${GH_API}/repos/${owner}/${repo}/forks`, {}, { headers: getHeaders() })
  return data
}

async function listBranches(owner, repo) {
  const { data } = await axios.get(`${GH_API}/repos/${owner}/${repo}/branches`, { headers: getHeaders() })
  return data
}
...
async function getReadme(owner, repo) {
  const { data } = await axios.get(`${GH_API}/repos/${owner}/${repo}/readme`, { headers: getHeaders() })
...
async function getFileSha(owner, repo, filePath) {
  try {
    const res = await axios.get(`${GH_API}/repos/${owner}/${repo}/contents/${filePath}`, { headers: getHeaders() })
    return res.data.sha
  } catch {
    return null
  }
...
async function uploadFile(owner, repo, filePath, content) {
  ...
  await axios.put(`${GH_API}/repos/${owner}/${repo}/contents/${filePath}`, body, { headers: getHeaders() })
}
...
export default {
  ...
  run: async (conn, m, { downloadM }) => {
    ...
    if (!global.githubToken) return m.reply('githubToken belum diset di config.js.')
    ...
    try {
      if (sub === 'upload') {
        ...
        if (!repoName) return m.reply('Format: .git upload <nama-repo> (reply file zip)')
          ...
          for (const entry of entries) {
            try {
              ...
              if (parts.length > 1) {
                ...
                if (topDirs.length === 1 && entries.every(e => e.entryName.startsWith(topDirs[0] + '/'))) {
      ...
      switch (sub) {
        ...
        default:
          return m.reply('Subcommand tidak dikenali. Ketik .git buat lihat menu.')
      }
    } catch (e) {
