...
export default {
  ...
  run: async (conn, m) => {
    ...
    try {
      ...
      await m.reply(
        `Mengambil daftar file dari ${owner}/${repoName}...`
      );
      ...
      if (!files.length) {
        await conn.sendMessage(m.chat, { react: { text: "✅", key: m.key } });
        return m.reply(`Repo ${repoName} sudah kosong, tidak ada file yang dihapus.`);
      }

      await m.reply(`Menghapus ${files.length} file dari ${owner}/${repoName}...`);
