...
async function uploadFileVercel(buffer) {
  await axios.post(VC_API + '/v2/files', buffer, {
    headers: {
      Authorization: 'Bearer ' + global.vercelToken,
...
async function ensureVercelProject(webName) {
  await axios.post(VC_API + '/v9/projects', { name: webName }, { headers: vcHeaders() }).catch(() => {});
}

async function createVercelDeployment(webName, files) {
  const res = await axios.post(VC_API + '/v13/deployments', {
    name: webName,
    project: webName,
    files: files.map(f => ({ file: f.filePath, sha: sha1(f.content), size: f.content.length })),
...
async function waitForReady(deploymentId, maxWait = 60000) {
  ...
  while (elapsed < maxWait) {
    ...
    elapsed += interval;
    const res = await axios.get(VC_API + '/v13/deployments/' + deploymentId, { headers: vcHeaders() });
    const state = res.data.readyState;
    if (state === 'READY') return res.data;
    if (state === 'ERROR' || state === 'CANCELED') throw new Error('Deployment state: ' + state);
  }
...
async function ghRepoExists(owner, repo) {
  try {
    await axios.get(GH_API + '/repos/' + owner + '/' + repo, { headers: ghHeaders() });
    return true;
  } catch {
    return false;
  }
...
async function createGhRepo(repo) {
  await axios.post(GH_API + '/user/repos', { name: repo, private: false, auto_init: false }, { headers: ghHeaders() });
}

async function getGhFileSha(owner, repo, filePath) {
  try {
    const res = await axios.get(GH_API + '/repos/' + owner + '/' + repo + '/contents/' + filePath, {
      headers: ghHeaders(),
      params: { ref: 'main' }
    });
    return res.data.sha;
...
async function upsertGhFile(owner, repo, filePath, content) {
  ...
  await axios.put(GH_API + '/repos/' + owner + '/' + repo + '/contents/' + filePath, body, { headers: ghHeaders() });
}

async function enablePages(owner, repo) {
  try {
    await axios.post(GH_API + '/repos/' + owner + '/' + repo + '/pages', { source: { branch: 'main', path: '/' } }, { headers: ghHeaders() });
  } catch (err) {
...
async function deployToGithub(conn, m, webName, files) {
  ...
  await send(conn, m,
    ...
    'Repo : https://github.com/' + ghUser + '/' + webName + '\n' +
    'URL  : ' + deployUrl + '\n\n' +
  ...
  run: async (conn, m, { downloadM, quoted }) => {
    ...
    if (!target || !['vercel', 'github'].includes(target)) {
      ...
      if (!m.isQuoted || !/zip|html/i.test(mime)) {
        return send(conn, m, 'Reply file ZIP atau HTML.\nContoh: .cweb namaweb');
      }

      const webName = m.text?.trim().toLowerCase().replace(/[^a-z0-9-_]/g, '');
      if (!webName) return send(conn, m, 'Sertakan nama website.\nContoh: .cweb namaweb');

      const buffer = await downloadM();
      pendingDeploy.set(m.sender, { buffer, mime, webName, timestamp: Date.now() });

      return new ButtonV2(conn)
        .setTitle('Deploy Website')
        .setSubtitle(webName)
        .setBody('File terdeteksi: ' + (/zip/i.test(mime) ? 'ZIP' : 'HTML') + '\nPilih platform tujuan deploy:')
        .setFooter('SbyuXd Deploy')
        .setThumbnail(global.thumbnailUrl || 'https://img2.pixhost.to/images/9130/746977018_sbyuxd.jpg')
        .addRawButton({
          ...
          nativeFlowInfo: {
            ...
            paramsJson: JSON.stringify({
              ...
              sections: [{
                ...
                highlight_label: '',
                rows: [
                  { header: '', title: 'Vercel', description: 'Cepat dan auto HTTPS', id: '.cweb vercel' },
                  { header: '', title: 'GitHub Pages', description: 'Gratis selamanya', id: '.cweb github' }
                ]
              }]
            })
          }
    ...
    if (!cached) {
      return send(conn, m, 'Sesi deploy kadaluarsa atau tidak ditemukan. Reply file lagi dari awal.');
    }
