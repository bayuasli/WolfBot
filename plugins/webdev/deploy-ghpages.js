...
export default {
  ...
  run: async (conn, m, { downloadM }) => {
    ...
    if (!quoted?.isMedia) {
      return m.reply("Reply file zip yang berisi struktur web.\n\nContoh: .ghpages nama-repo");
    }
    ...
    if (!/^[a-zA-Z0-9_.-]+$/.test(repoName)) {
      return m.reply("Nama repo tidak valid. Gunakan huruf, angka, titik, atau strip saja.");
    }
    ...
    try {
      ...
      if (topDirs.length === 1 && fs.statSync(path.join(extractPath, topDirs[0])).isDirectory()) {
