...
export default {
  ...
  run: async (conn, m, { downloadM }) => {
    try {
      if (!m.isQuoted || !m.quoted.isMedia) return m.reply('Reply media berupa gambar/video.')
      ...
      const mime = m.quoted?.msg?.mimetype || m.quoted?.mimetype || 'image/jpeg'
      const ext = mime.split('/')[1] || 'jpg'
      const sizeKB = (buffer.length / 1024).toFixed(2)
      const sizeMB = (buffer.length / (1024 * 1024)).toFixed(2)
      const fileSize = parseFloat(sizeMB) >= 1 ? `${sizeMB} MB` : `${sizeKB} KB`
      ...
      await conn.sendButton(m.chat, {
        ...
        buttons: [
          ...
          { name: 'cta_url', buttonParamsJson: JSON.stringify({ display_text: '🌐 Buka URL', url }) }
        ]
      }, { quoted: m })
