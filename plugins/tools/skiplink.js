export default {
  ...
  run: async (conn, m, { Func }) => {
    const url = m.args[0];
    if (!url) return m.reply('Masukkan URL yang ingin di-bypass.\nContoh: .bypass https://linkvertise.com/...');
      ...
      if (e.message && (e.message.includes('401') || e.message.includes('403'))) {
        try {
          ...
          if (result.status === 'success' && result.result) {
            ...
            return m.reply(`Gagal setelah refresh: ${result.message || 'Unknown error'}`);
          }
        } catch (e2) {
...
async function createSession() {
  ...
  const deviceId = crypto.default.createHash('sha256').update(`bypasstools:${androidId}`).digest('hex');
