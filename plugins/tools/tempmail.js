  ...
  run: async (conn, m) => {
    try {
          ...
          sections: [
            {
              title: '📧 Email',
              rows: [
                { title: 'Buat Email Baru', description: 'Generate email sementara 10 menit', id: '.tmail new' },
                { title: 'Cek Inbox', description: 'Lihat email masuk', id: '.tmail inbox' },
                { title: 'Info Sesi', description: 'Lihat email aktif + sisa waktu', id: '.tmail info' },
                { title: 'Stop Sesi', description: 'Tutup sesi email aktif', id: '.tmail stop' }
              ]
            }
          ]
        }, { quoted: m })
      ...
      if (sub === 'new') {
        ...
        const interval = setInterval(async () => {
          try {
            if (Date.now() > expireAt) {
              ...
              await sendQuickReply(conn, m.chat, {
                title: '⏰ Sesi TempMail Berakhir',
                body: `Email *${email}* sudah tidak aktif.`,
                footer: global.nameown || 'SbyuXd',
                buttons: [
                  { text: '📧 Buat Email Baru', id: '.tmail new' }
                ]
              }, { quoted: m })
              return
            }
        ...
        await sendMixedButton(conn, m.chat, {
      ...
      if (sub === 'inbox') {
        if (!session) return sendQuickReply(conn, m.chat, {
        ...
        const list = messages.slice(-5).map((msg, i) =>
          `*${i + 1}.* ${msg.subject || '-'}\n` +
      ...
      if (sub === 'info') {
        ...
        const sisa = Math.max(0, Math.floor((session.expireAt - Date.now()) / 1000))
