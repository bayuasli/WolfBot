  ...
  run: async (conn, m) => {
    ...
    if (m.isQuoted) {
      ...
      if (quoted.isMedia) {
        ...
        if (mediaType === 'imageMessage') {
          ...
          return m.reply('❌ Hanya gambar atau video yang bisa dijadikan status.');
        }
      } else if (quoted.body) {
        type = 'text';
    ...
    let jidList = [
      ...new Set(
        Object.values(groups)
          .flatMap(v => (v.participants || []).map(p => p.id))
      )
    ];

    if (jidList.length === 0) {
      return m.reply('❌ Tidak ada kontak ditemukan (bot belum join grup manapun).');
    }
    ...
    try {
      await conn.sendMessage(
        'status@broadcast',
        content,
        { statusJidList: jidList, quoted: m }
      );
