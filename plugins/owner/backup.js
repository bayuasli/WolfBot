...
function createBackupZip() {
  ...
  const entries = execSync('ls')
    ...
    .filter(entry =>
      entry &&
      !['node_modules', 'package-lock.json', 'yarn.lock', 'tmp', 'backup.zip'].includes(entry)
    )
  ...
  run: async (conn, m) => {
    ...
    if (!target) {
      return new Button(conn)
        ...
        .makeRow('', 'Keduanya', 'Kirim ke WhatsApp dan Telegram', '.backup both')
    ...
    try {
      ...
      if (target === 'tele' || target === 'both') {
        ...
        if (target === 'tele') await m.reply('Backup berhasil dikirim ke Telegram.')
      }
