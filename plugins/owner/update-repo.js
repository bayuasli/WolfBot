...
const REPO_OWNER = 'bayuasli'
const REPO_NAME = 'SbyuxdBot-V2'
const BRANCH = 'main'
...
export default {
  ...
  run: async (conn, m) => {
    ...
    if (action === 'delete' || action === 'del' || action === 'rm') {
      ...
      if (!filePath) {
        return m.reply('Sertakan path file yang akan dihapus.\n\nContoh: .upsc delete plugins/test.js')
      }
      ...
      try {
        ...
        return m.reply(`✅ *File berhasil dihapus*\n\n📄 ${cleanPath}\n📦 Repo: ${REPO_OWNER}/${REPO_NAME}`)
      } catch (error) {
        console.error('Delete error:', error)
        return m.reply(`❌ Gagal menghapus file: ${error.response?.data?.message || error.message}`)
      }
    }
    ...
    if (action === 'list' || action === 'ls') {
      ...
      await m.reply(`📂 Mengambil daftar file dari *${cleanPath || 'root'}*...`)
      
      try {
        const files = await listFiles(cleanPath, token)
        
        if (files.length === 0) {
          return m.reply(`📁 *${cleanPath || 'root'}*\n\nFolder kosong atau tidak ditemukan`)
        }
        ...
        return m.reply(`❌ Gagal mengambil daftar: ${error.response?.data?.message || error.message}`)
      }
    }
    ...
    if (action === 'info') {
      ...
      try {
        ...
        return m.reply(`❌ Gagal mengambil info: ${error.response?.data?.message || error.message}`)
      }
    }
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      return m.reply(`❌ Tidak bisa upload folder.\nGunakan .upsc list ${repoPath} untuk melihat isi folder`)
    }
    
    await m.reply(`📤 Mengupload *${repoPath}* ke GitHub...\n⏳ Mohon tunggu...`)
    ...
    try {
      ...
      const actionText = result.action === 'created' ? 'membuat file baru' : 'menimpa file'
