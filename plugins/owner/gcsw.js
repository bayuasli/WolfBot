...
export default {
  ...
  run: async (conn, m) => {
    ...
    if (sessions.has(m.sender) && input && /^[\d,\s]+$/.test(input)) {
      ...
      const indices = [...new Set(input.split(',').map(n => parseInt(n.trim()) - 1))];
      ...
      for (const target of targets) {
        try {
          ...
          const msg = generateWAMessageFromContent(target.id, {
            messageContextInfo: { messageSecret },
            groupStatusMessageV2: {
              message: { ...inside, messageContextInfo: { messageSecret } }
            }
          }, {});
          await conn.relayMessage(target.id, msg.message, { messageId: msg.key.id });
    ...
    const groups = Object.entries(groupsData).map(([id, data]) => ({ id, subject: data.subject }));
    ...
    const listText = `Terdapat ${groups.length} Grup Aktif.\n\n` + 
