...
const presets = { qtext, metaai, pixx, pay, pack, poll, vn, gif, gc, video, loc, kontak, salr, order }
...
export default {
  ...
  run: async (conn, m) => {
    ...
    if (!sub || sub === 'list') {
      const list = Object.keys(presets).map((k, i) => `${i + 1}. ${k}`).join('\n')
      return m.reply(`*List Fake Quoted*\n\n${list}\n\nCara pakai: .fq get <nama>\nContoh: .fq get poll`)
    }
    ...
    if (sub === 'get') {
      ...
      if (!preset) return m.reply(`Preset *${name}* tidak ditemukan.\nKetik .fq list untuk melihat daftar.`)

      await conn.sendMessage(m.chat, { text: m.body || '.' }, { quoted: preset })
      return
    }
