...
export default {
  ...
  run: async (conn, m) => {
    ...
    if (sub === 'addid' || sub === 'addidch') {
      const id = rest.trim();
      if (!id) return m.reply('Masukkan channel ID\nContoh: ' + prefix + 'upch addid 120363423327724431@newsletter');
    ...
    if (sub === 'listid' || sub === 'listidch') {
      const channels = loadChannels();
      if (channels.length === 0) return m.reply('Belum ada channel ID tersimpan.');
    ...
    if (sub === 'delid' || sub === 'delidch') {
      ...
      if (!input) return m.reply('Masukkan nomor channel yang akan dihapus\nContoh: ' + prefix + 'upch delid 1,2,3');

      const channels = loadChannels();
      const indices = input.split(',').map(n => parseInt(n.trim()) - 1).filter(i => !isNaN(i) && i >= 0 && i < channels.length);
    ...
    if (channels.length === 0) {
      return m.reply('Belum ada channel ID. Tambahkan dulu:\n' + prefix + 'upch addid <id>');
    }
