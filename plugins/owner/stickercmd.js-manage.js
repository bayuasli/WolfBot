...
import { getSticker, setSticker, lockSticker, deleteSticker, listStickers } from '#lib/core/stickercmd.js'
  ...
  run: async (conn, m) => {
    if (m.command === 'setcmd') {
      if (!m.isQuoted || m.quoted.type !== 'stickerMessage') {
        return m.reply(`Balas sticker dengan perintah *${m.prefix}${m.command}*`)
      }
    ...
    if (m.command === 'lockcmd' || m.command === 'unlockcmd') {
      if (!m.isQuoted || m.quoted.type !== 'stickerMessage') return m.reply('Reply sticker!')
      if (!m.quoted.msg?.fileSha256) return m.reply('SHA256 Hash Missing')
    ...
    if (m.command === 'delcmd') {
      if (!m.isQuoted || m.quoted.type !== 'stickerMessage') return m.reply('Reply sticker yang mau dihapus commandnya!')
    ...
    if (m.command === 'listcmd') {
      ...
      if (stickers.length === 0) return m.reply('Belum ada sticker command tersimpan.')

      const lines = stickers.map((s, i) =>
