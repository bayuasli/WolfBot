...
const bufferToWebp = (inputBuffer) => new Promise((resolve, reject) => {
  const tmpIn = path.join(temp, crypto.randomBytes(6).readUIntLE(0, 6).toString(36) + '.tmp');
  const tmpOut = path.join(temp, crypto.randomBytes(6).readUIntLE(0, 6).toString(36) + '.webp');
...
const writeExif = async (buffer, metadata) => {
  ...
  const json = {
    ...
    'ios-app-store-link': 'https://apps.apple.com/id/app/muslim-pro-al-quran-adzan/id388389451',
    emojis: ['😋', '😎', '🤣'],
    'is-avatar-sticker': 0
  };
...
const toB64Url = (buffer) => Buffer.from(buffer).toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/g, '');
...
const uploadToServer = async (conn, buffer, { hkdf, mediaPath, mediaKey = crypto.randomBytes(32) }) => {
...
const sendStickerPack = async (conn, m, pack, packNumber = 1, totalPacks = 1) => {
  const zip = new JSZip();
  ...
  await conn.relayMessage(m.chat, {
    ...
    stickerPackMessage: {
      ...
      packDescription: 'Stiker pack dari Z3PH BOT - ' + pack.length + ' stiker',
...
export default {
  ...
  run: async (conn, m) => {
    try {
      ...
      const res = await fetch(TG_BOT + '/getStickerSet?name=' + encodeURIComponent(packName));
      if (!res.ok) throw new Error('Gagal ambil data stiker: ' + res.status);

      const json = await res.json();
      if (!json.ok || !json.result?.stickers?.length) throw new Error('Data stiker tidak valid.');
      ...
      for (const stickerData of stickers) {
        try {
          const fileRes = await fetch(TG_BOT + '/getFile?file_id=' + stickerData.file_id);
          if (!fileRes.ok) continue;

          const fileJson = await fileRes.json();
          if (!fileJson.ok || !fileJson.result?.file_path) continue;

          const imgRes = await fetch('https://api.telegram.org/file/bot' + token + '/' + fileJson.result.file_path);
          ...
          allPack.push({ buffer: stickerBuffer, ext: 'webp', mimetype: 'image/webp', isAnimated, isLottie: false });

          await delay(300);
        } catch { continue; }
      }

      if (!allPack.length) throw new Error('Tidak ada stiker yang berhasil diproses.');
      ...
      await conn.sendMessage(m.chat, { react: { text: '✅', key: m.key } });
      await m.reply('Berhasil mengirim ' + allPack.length + ' stiker dalam ' + totalPacks + ' pack.');
