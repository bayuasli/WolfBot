...
export default {
  name: 'spriv',
  category: 'sticker',
  command: ['anticolong', 'spriv', 'sprivat'],
  alias: ['setattr', 'stattr'],
  ...
  run: async (conn, m, { quoted }) => {
    try {
      ...
      const contextInfo = m.msg?.contextInfo || m.quoted?.contextInfo || {};
      const quotedMessage = contextInfo.quotedMessage || m.quoted?.message || m.quoted;
      ...
      if (stickerMessage.mimetype !== 'image/webp') {
        return m.reply('Ini bukan sticker WEBP biasa. Untuk lottie/application berbeda caranya.');
      }
      ...
      const metadata = {
        ...
        'sticker-pack-name': 'sbyuxD [ Develop]',
      ...
      return m.reply('Gagal inject atribut sticker. Cek console.');
    }
  }
};