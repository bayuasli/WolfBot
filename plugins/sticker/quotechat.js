...
export default {
  ...
  run: async (conn, m) => {
    ...
    const [username, avatar] = await Promise.all([
      Promise.resolve(m.isQuoted ? (await conn.getName(target) || target.split("@")[0]) : (m.pushname || m.sender.split("@")[0])),
      conn.profilePictureUrl(target, "image").catch(() => "https://telegra.ph/file/6880771a42bad09dd6087.jpg")
    ]);
    ...
    try {
      ...
      return m.reply("Gagal membuat quote: " + (err?.response?.data?.message || err.message));
    }
  }
};