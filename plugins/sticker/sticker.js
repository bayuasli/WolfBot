async function getAlbumMedia(conn, chat, albumId, expectedCount, timeoutMs = 15000) {
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    for (const entry of chatMsgs) {
      const parentId = entry.message?.messageContextInfo?.messageAssociation?.parentMessageKey?.id
...
export default {
  ...
  run: async (conn, m, { quoted }) => {
    const albumSource =
      m.type === 'albumMessage' ? { id: m.key.id, msg: m.msg } :
      m.quoted?.type === 'albumMessage' ? { id: m.quoted.id, msg: m.quoted.msg } :
      null

    if (albumSource) {
      const expectedCount = (albumSource.msg.expectedImageCount || 0) + (albumSource.msg.expectedVideoCount || 0)
      const items = await getAlbumMedia(conn, m.chat, albumSource.id, expectedCount)

      if (items.length === 0) {
        return m.reply('Media album belum diterima semua atau sudah kadaluarsa dari histori bot.')
      }
    ...
    if (quoted && /image|video|webp/.test(quoted.msg?.mimetype)) {
      ...
      if (quoted.msg?.seconds > 10) {
        return m.reply('Video lebih dari 10 detik tidak bisa dijadikan sticker.')
      }
    ...
    m.reply('Reply gambar/video atau kirim album gambar untuk dijadikan sticker.\n\nUntuk custom packname: .s packname|author')
  }
}