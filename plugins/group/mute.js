  ...
  run: async (conn, m) => {
    ...
    if (command === 'mute') {
      ...
      if (!args || args.length === 0) {
        ...
        return m.reply('Gunakan: .mute 1,2,3 (dari list .mutels) atau ketik .mute langsung di grup.');
      }

      const indices = args.join(',').split(',').map(v => parseInt(v.trim()) - 1).filter(v => !isNaN(v) && v >= 0);
      const toMute = indices.map(i => groups[i]).filter(g => g && !muted.includes(g.id));

      if (toMute.length === 0) {
        return m.reply('Tidak ada grup yang bisa di-mute (mungkin sudah di-mute atau index salah).');
      }
    ...
    if (command === 'unmute') {
      ...
      if (!args || args.length === 0) {
        return m.reply('Gunakan: .unmute 1,2,3 (dari list .muted)\n\nCek list grup yang di-mute dengan .muted');
      }
      ...
      const indices = args.join(',').split(',').map(v => parseInt(v.trim()) - 1).filter(v => !isNaN(v) && v >= 0);
      const toUnmute = indices.map(i => mutedGroups[i]).filter(g => g && g.id);

      if (toUnmute.length === 0) {
        return m.reply('Tidak ada grup yang bisa di-unmute (mungkin index salah).\n\nCek list dengan .muted');
      }
      ...
      return m.reply('🔊 Berhasil unmute ' + toUnmute.length + ' grup:\n' + names);
    }
  }
};