export default {
  ...
  run: async (conn, m) => {
    try {
      ...
      const formatDate = ts => new Intl.DateTimeFormat('id-ID', {
        weekday: 'long', day: 'numeric', month: 'long', year: 'numeric'
      }).format(new Date(ts * 1000))

      const list = await conn.groupRequestParticipantsList(m.chat)

      if (!sub) return m.reply(
        `*[ REQUEST JOIN GROUP ]*\n\n` +
      ...
      if (!['approve', 'reject'].includes(sub)) return m.reply('Sub-command tidak dikenal.')
      if (!list.length) return m.reply('Tidak ada permintaan bergabung.')
      ...
      if (opt === 'all') {
        ...
        return m.reply(`*${label} semua permintaan bergabung (${list.length} orang).*`)
      }

      const indexes = opt?.split('|').map(n => parseInt(n.trim()) - 1).filter(n => !isNaN(n))
      if (!indexes?.length) return m.reply('Masukkan nomor urut dari .acc list.\nContoh: .acc approve 1|2|3')

      const targets = indexes.map(i => list[i]).filter(Boolean)
      if (!targets.length) return m.reply('Nomor urut tidak ditemukan di daftar.')

      let res = ''
      for (const [i, r] of targets.entries()) {
        const result = await conn.groupRequestParticipantsUpdate(m.chat, [r.jid], sub)
        const status = result[0]?.status === 'success' ? '✅ Berhasil' : '❌ Gagal'
