...
export default {
  ...
  run: async (conn, m) => {
    if (m.command === 'topmem') {
      ...
      if (!members?.length) return m.reply('Belum ada data aktivitas pesan di grup ini.');
    ...
    if (m.command === 'topgc') {
      ...
      if (!groupIds.length) return m.reply('Bot belum bergabung di grup manapun.');
      ...
      if (!stats?.length) return m.reply('Belum ada data aktivitas pesan dari grup yang diikuti.');
