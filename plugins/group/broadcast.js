...
function getTargetGroups(conn) {
  return getAllGroups(conn).filter(([jid]) => !state.filterGroups.includes(jid));
}
...
function startScheduler(conn) {
  ...
  state.scheduler = setInterval(async () => {
    ...
    await sendBroadcast(conn, targets, `${state.text}\n\n_Dikirim otomatis pukul ${timeLabel}_`);
  }, 60000);
}
  ...
  run: async (conn, m) => {
    ...
    switch (sub.toLowerCase()) {
      case "set": {
        ...
        if (!text) return m.reply("Sertakan teks broadcast.\n\nContoh: .bc set Halo semua!");
        state.text = text;
        saveDB();
        return m.reply(`Teks broadcast berhasil diset.\n\nPreview:\n${state.text}`);
      }
      ...
      case "list": {
        ...
        if (!groups.length) return m.reply("Bot tidak bergabung di grup manapun.");
      ...
      case "send": {
        if (!state.text) return m.reply("Teks broadcast belum diset. Gunakan .bc set <teks> terlebih dahulu.");
        const targets = getTargetGroups(conn);
        if (!targets.length) return m.reply("Tidak ada grup target yang tersedia.");

        await m.reply(`Mengirim broadcast ke ${targets.length} grup...`);
        const { success, failed } = await sendBroadcast(conn, targets, state.text);
      ...
      case "filter": {
        ...
        switch (action.toLowerCase()) {
          case "add": {
            ...
            if (!input) return m.reply("Sertakan nomor urut grup.\n\nContoh: .bc filter add 1,2,5");
            ...
            if (invalid.length) {
              return m.reply(`Nomor tidak valid: ${invalid.map(i => i + 1).join(", ")}.\nGunakan .bc list untuk melihat daftar grup.`);
            }
            ...
            if (!added.length) return m.reply("Semua grup yang dipilih sudah ada di blacklist.");

            saveDB();
            return m.reply(`${added.length} grup berhasil ditambahkan ke blacklist:\n\n${added.map((n, i) => `${i + 1}. ${n}`).join("\n")}`);
          }
          ...
          case "remove": {
            ...
            if (!removed.length) return m.reply("Grup yang dipilih tidak ada di blacklist.");

            saveDB();
            return m.reply(`${removed.length} grup berhasil dihapus dari blacklist:\n\n${removed.map((n, i) => `${i + 1}. ${n}`).join("\n")}`);
          }

          case "list": {
            if (!state.filterGroups.length) return m.reply("Blacklist kosong. Broadcast dikirim ke semua grup.");
            ...
            return m.reply(`*Grup Blacklist (${state.filterGroups.length})*\n\n${lines.join("\n\n")}`);
          }

          case "clear": {
            state.filterGroups = [];
            saveDB();
            return m.reply("Blacklist dihapus. Broadcast akan dikirim ke semua grup.");
          }

          default:
            return m.reply("Aksi tidak dikenal. Gunakan: add, remove, list, atau clear.");
        }
      }
