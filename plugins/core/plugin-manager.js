...
export default {
  ...
  run: async (conn, m) => {
    ...
    if (!query) {
      return m.reply('Gunakan: .infoplug <nama plugin>\nContoh: .infoplug menu');
    }
    ...
    if (!foundPlugin) {
      const list = Object.values(plugins).map(p => p.name || 'unknown').join(', ');
      return m.reply('Plugin "' + query + '" tidak ditemukan.\n\nDaftar plugin: ' + list);
    }
    ...
    const cmds = Array.isArray(foundPlugin.command) ? foundPlugin.command.join(', ') : foundPlugin.command || '-';
    const aliases = Array.isArray(foundPlugin.alias) ? foundPlugin.alias.join(', ') : foundPlugin.alias || '-';
    ...
    const settingsText = activeSettings.length > 0 ? activeSettings.join(', ') : 'None';
...
[Z3PHRINE](!https://t.me/Z3PHRINE)
    `)
        ...
        .addCode(
          ...
          '  name: "' + foundName + '",\n' +
          '  category: "' + category + '",\n' +
          '  command: [' + cmds.split(', ').map(c => '"' + c + '"').join(', ') + '],\n' +
          '  settings: { ' + activeSettings.map(s => s.toLowerCase() + ': true').join(', ') + ' }\n' +
          '}'
        )
        .addTable([
          ['Property', 'Value'],
