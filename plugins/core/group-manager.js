...
export default {
  ...
  run: async (conn, m) => {
    ...
    if (!sub) {
      return m.reply(
        'GROUP MANAGER\n\n' +
        '• ' + prefix + 'gm create nama\n' +
        '• ' + prefix + 'gm leave\n' +
        '• ' + prefix + 'gm setname nama baru\n' +
        '• ' + prefix + 'gm setdesc deskripsi baru\n' +
        '• ' + prefix + 'gm link\n' +
        '• ' + prefix + 'gm revoke\n' +
        '• ' + prefix + 'gm ephemeral <detik>\n' +
        '• ' + prefix + 'gm announce <on/off>\n' +
        '• ' + prefix + 'gm restrict <on/off>\n' +
        '• ' + prefix + 'gm addmode <on/off>\n' +
        '• ' + prefix + 'gm joinmode <on/off>\n' +
        '• ' + prefix + 'gm info\n' +
        '• ' + prefix + 'gm members\n' +
        '• ' + prefix + 'gm admins\n' +
        '• ' + prefix + 'gm save\n' +
    ...
    try {
      ...
      if (sub === 'save') {
        ...
        return m.reply(
          ...
          'Total Admin: ' + participants.filter(p => p.admin).length
        )
      }

      if (sub === 'list') {
        const entries = Object.entries(groups)
        if (entries.length === 0) return m.reply('Belum ada data grup tersimpan.')
      ...
      if (sub === 'get') {
        ...
        let text =
          ...
          'ID: ' + data.id + '\n' +
          'Nama: ' + data.subject + '\n' +
          'Deskripsi: ' + (data.desc || 'Tidak ada') + '\n' +
          'Owner: ' + (data.owner || 'Tidak diketahui') + '\n' +
          'Total Member: ' + data.totalMembers + '\n' +
          'Total Admin: ' + data.admins + '\n' +
          'Ephemeral: ' + data.ephemeral + ' detik\n' +
          'Announce: ' + (data.announce ? 'ON' : 'OFF') + '\n' +
          'Restrict: ' + (data.restrict ? 'ON' : 'OFF') + '\n' +
          'Dibuat: ' + new Date(data.createdAt).toLocaleString() + '\n' +
          'Disimpan: ' + data.savedAt + '\n\n' +
          'MEMBER LIST (' + data.totalMembers + ')\n'
      ...
      if (sub === 'count') {
        ...
        return m.reply(
          'STATISTIK GRUP\n\n' +
          'Total Grup Tersimpan: ' + Object.keys(groups).length + '\n' +
          'Total User Unik: ' + totalUsers.size
        )
      }

      if (!isGroup && sub !== 'create' && sub !== 'join' && sub !== 'list' && sub !== 'get' && sub !== 'count') {
      ...
      if (sub === 'create') {
        ...
        return m.reply(
          ...
          'Nama: ' + name + '\n' +
          'JID: ' + metadata.id + '\n' +
          'Link: https://chat.whatsapp.com/' + (await conn.groupInviteCode(metadata.id))
        )
      }
      ...
      if (sub === 'setdesc') {
        ...
        if (!desc) return m.reply('Format: ' + prefix + 'gm setdesc deskripsi_baru')
      ...
      if (sub === 'ephemeral') {
        ...
        if (!seconds || seconds < 0) return m.reply('Format: ' + prefix + 'gm ephemeral <detik>\nContoh: 86400 = 1 hari')
      ...
      if (sub === 'announce') {
        ...
        if (!['on', 'off'].includes(value)) return m.reply('Format: ' + prefix + 'gm announce on/off')

        await conn.groupSettingUpdate(chatId, value === 'on' ? 'announcement' : 'not_announcement')
        return m.reply('Mode pengumuman: ' + (value === 'on' ? 'ON' : 'OFF'))
      }

      if (sub === 'restrict') {
        const value = rest.toLowerCase()
        if (!['on', 'off'].includes(value)) return m.reply('Format: ' + prefix + 'gm restrict on/off')

        await conn.groupSettingUpdate(chatId, value === 'on' ? 'locked' : 'unlocked')
        return m.reply('Mode restrict: ' + (value === 'on' ? 'ON' : 'OFF'))
      }

      if (sub === 'addmode') {
        const value = rest.toLowerCase()
        if (!['on', 'off'].includes(value)) return m.reply('Format: ' + prefix + 'gm addmode on/off')

        await conn.groupMemberAddMode(chatId, value === 'on' ? 'admin_add' : 'all_member_add')
        return m.reply('Mode add member: ' + (value === 'on' ? 'Admin Only' : 'All Members'))
      }

      if (sub === 'joinmode') {
        const value = rest.toLowerCase()
        if (!['on', 'off'].includes(value)) return m.reply('Format: ' + prefix + 'gm joinmode on/off')

        await conn.groupJoinApprovalMode(chatId, value === 'on' ? 'on' : 'off')
        return m.reply('Mode approval join: ' + (value === 'on' ? 'ON' : 'OFF'))
      }
      ...
      if (sub === 'info') {
        ...
        return m.reply(
          ...
          'Owner: ' + (metadata.owner || 'Tidak diketahui') + '\n' +
          'Total Member: ' + (metadata.participants?.length || 0) + '\n' +
          'Total Admin: ' + (metadata.participants?.filter(p => p.admin).length || 0) + '\n' +
          'Ephemeral: ' + (metadata.ephemeralDuration || 0) + ' detik'
        )
      }
      ...
      return m.reply('Subcommand tidak dikenal. Ketik ' + prefix + 'gm untuk lihat daftar perintah.')
