...
export default {
  ...
  run: async (conn, m) => {
    ...
    if (m.command === 'listprefix') {
      return m.reply(`Prefix aktif saat ini:\n${prefixes.map(p => `• ${p}`).join('\n')}`)
    }
    ...
    if (m.command === 'addprefix') {
      ...
      if (prefixes.includes(newPrefix)) return m.reply(`Prefix "${newPrefix}" sudah terdaftar.`)
    ...
    if (m.command === 'delprefix') {
      ...
      if (!prefixes.includes(target)) return m.reply(`Prefix "${target}" tidak ditemukan.`)
      if (prefixes.length <= 1) return m.reply('Minimal harus ada 1 prefix aktif, tidak bisa dihapus semua.')
