...
async function ttdown(url) {
  if (!url.includes('tiktok.com')) throw new Error('URL tidak valid, harus link TikTok.')
...
export default {
  ...
  run: async (conn, m, { Func }) => {
    try {
      ...
      if (!result.downloads.length) throw new Error('Gagal ambil link download.')

      const noWatermark = result.downloads.find(d => d.type === 'mp4' && /no watermark/i.test(d.label))
      const video = noWatermark || result.downloads.find(d => d.type === 'mp4') || result.downloads[0]
