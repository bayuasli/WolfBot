...
export default {
  ...
  run: async (conn, m, { Func }) => {
    try {
      ...
      if (!query) return m.reply('Masukkan kata kunci.\nContoh: .ttsearch chess jj kece')
      ...
      if (!videos || !videos.length) throw new Error('Tidak ada video ditemukan.')

      const first = videos[0]
      const tiktokUrl = `https://www.tiktok.com/@${first.author.unique_id}/video/${first.video_id}`

      const result = await ttdown(tiktokUrl)
      const noWatermark = result.downloads.find(d => /no watermark/i.test(d.label)) || result.downloads.find(d => d.type === 'mp4') || result.downloads[0]
