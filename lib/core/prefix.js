import fs from 'fs'
import path from 'path'

const dbPath = path.join(process.cwd(), 'lib/database/prefix.json')
const defaultPrefixes = ['.', '!', '#', '/', '?', '°', '×', '÷', '€', '+', '=', '~', '^', '%', '@', '|']

function ensureDb() {
  const dir = path.dirname(dbPath)
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true })
  if (!fs.existsSync(dbPath)) fs.writeFileSync(dbPath, JSON.stringify({ prefixes: defaultPrefixes }, null, 2))
}

export function loadPrefixes() {
  ensureDb()
  try {
    const data = JSON.parse(fs.readFileSync(dbPath, 'utf-8'))
    return Array.isArray(data.prefixes) && data.prefixes.length ? data.prefixes : defaultPrefixes
  } catch {
    return defaultPrefixes
  }
}

export function savePrefixes(prefixes) {
  ensureDb()
  fs.writeFileSync(dbPath, JSON.stringify({ prefixes }, null, 2))
}

function escapeForCharClass(char) {
  return char.replace(/[\^\]\\-]/g, '\\$&')
}

export function getPrefixRegex() {
  const prefixes = loadPrefixes()
  const escaped = prefixes.map(escapeForCharClass).join('')
  return new RegExp(`^[${escaped}]`, 'i')
}