...
class ThemeManager {
  ...
  async setTitle(title) {
    try {
      ...
      return { data: 'Title berhasil diupdate: ' + this.config.title };
    } catch (e) {
  ...
  async setDescription(description) {
    try {
      ...
      await this.saveConfig();
      return { data: 'Description berhasil diupdate: ' + this.config.description };
    } catch (e) {
      return { error: e.message };
    }
  }
  ...
  async setUrl(url) {
    try {
      ...
      await this.saveConfig();
      return { data: 'URL berhasil diupdate: ' + this.config.url };
    } catch (e) {
      return { error: e.message };
    }
  }
  ...
  async setFavicon(faviconData) {
    try {
      if (!faviconData) {
        ...
        await this.saveConfig();
        return { data: 'Favicon berhasil dihapus' };
      }

      const required = ['thumbnailDirectPath', 'thumbnailSha256', 'thumbnailEncSha256', 'mediaKey', 'mediaKeyTimestamp'];
      ...
      await this.saveConfig();
      return { data: 'Favicon berhasil diupdate' };
    } catch (e) {
      return { error: e.message };
    }
  }
