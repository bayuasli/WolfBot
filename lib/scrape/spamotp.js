...
export async function sendRequest(endpoint) {
  ...
  for (let attempt = 0; attempt <= CONFIG.retries; attempt++) {
    try {
      ...
      return { success: false, name: endpoint.name, status, message: resp.data?.message || 'Gagal' };
      ...
      return { success: false, name: endpoint.name, status: err.response?.status || 0, message: err.message };
    }
  }
