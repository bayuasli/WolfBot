 ...
 * }, { quoted: m })
 */
export const sendLocationButton = async (conn, jid, content = {}, options = {}) => {
  return relay(conn, jid, {
    interactiveMessage: {
 ...
 * }, { quoted: m })
 */
export const sendMixedButton = async (conn, jid, content = {}, options = {}) => {
  const header = await getMedia(conn, content)
  const buttons = (content.buttons || []).map(b => {
    if (b.type === 'reply') return {
      name: 'quick_reply',
      buttonParamsJson: JSON.stringify({ display_text: b.text || b.label, id: b.id })
    }
    if (b.type === 'url') return {
      name: 'cta_url',
      buttonParamsJson: JSON.stringify({ display_text: b.text || b.label, url: b.url, webview_interaction: b.webview || false })
    }
    if (b.type === 'copy') return {
      name: 'cta_copy',
      buttonParamsJson: JSON.stringify({ display_text: b.text || b.label, copy_code: b.code })
    }
    if (b.type === 'call') return {
      name: 'cta_call',
      buttonParamsJson: JSON.stringify({ display_text: b.text || b.label, id: b.number || b.id })
    }
 ...
 * }, { quoted: m })
 */
export const sendAddressButton = async (conn, jid, content = {}, options = {}) => {
  return relay(conn, jid, {
    interactiveMessage: {
      header: { title: content.title || '' },
      body: { text: content.body || content.text || '' },
