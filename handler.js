...
const { commandMap, onPlugins } = buildCommandIndex(plugins)
...
if (!isOwner && isBanned(m.sender)) return
  ...
  const ctx = { Func, downloadM, quoted, metadata, isOwner, isAdmin, isBotAdmin }
  ...
  if (m.prefix && body.startsWith(m.prefix)) {
    ...
    const match = Object.values(plugins).find(pl => {
      if (!pl?.command) return false
      const cmds = Array.isArray(pl.command) ? pl.command : [pl.command]
      const aliases = Array.isArray(pl.alias) ? pl.alias : pl.alias ? [pl.alias] : []
      return cmds.includes(firstWord) || aliases.includes(firstWord)
    })
  ...
  try {
    ...
    await m.reply('```ERROR WAK, GTW KENAWHY``` 😂')
  }
}